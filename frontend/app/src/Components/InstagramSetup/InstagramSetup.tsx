  ...
  return (
    <Grid container spacing={2}>
      {status === 'fb-not-set-up' && (
        <Grid item xs={12}>
          <Alert variant="standard" severity="warning">
            Facebook has not been set up yet, please set up your Facebook page
      ...
      {status === 'ig-not-set-up' && page && (
        <Grid item xs={12}>
          <Alert variant="standard" severity="info">
            The displayed Instagram account below is based on what is connected
            to your Facebook page. If the account you are trying to set up isn't
            the same, please look into{' '}
            <a
              target="_blank"
              href="https://help.instagram.com/176235449218188"
              rel="noreferrer"
      ...
      {(status === 'ig-not-set-up' || status === 'active') && !page && (
        <Grid item xs={12}>
          <Alert variant="standard" severity="error">
            You either have not provided access to any Instagram account when
            setting up your Facebook page or do not have an Instagram account
            associated with the Facebook page to begin with. Please fix this
            issue and set up Facebook again with the proper permissions.
          </Alert>
        </Grid>
      )}
      ...
      {facebookStage === 'inactive' && (
        <Grid item xs={12}>
          <Alert variant="standard" severity="warning">
            ...
            setup relies on Facebook, please refresh your Facebook token and try
            again.
          </Alert>
        </Grid>
      )}
      ...
      {status === 'active' && page && page!.id !== connectedPageId && (
        <Grid item xs={12}>
          <Alert variant="standard" severity="info">
            We have detected a different account to be set up than the one that
            is already connected. You may connect it below.
          </Alert>
        </Grid>
      )}
