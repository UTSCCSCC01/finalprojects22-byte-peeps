...
export const getSettings: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (user?.facebookApi == undefined) {
      return res.send({ status: 'fb-not-set-up', page: null, connectedPageId: null });

    } else if (!user.facebookApi.isActive) {
      return res.send({ status: 'inactive', page: null, connectedPageId: null });
      ...
      return res.send({ status: 'ig-not-set-up', page: page, connectedPageId: null });

    } 
      
    const pageId = await api.getPageIdByToken(user.facebookApi.token);
    const page = await api.getPage(user.facebookApi.token, pageId);
    const connectedPage = await api.getPage(user.facebookApi.token, user.instagramApi.nodeId);
    return res.send({ status: 'active', page: page, connectedPageId: connectedPage?.id ?? null });
...
export const connectPage: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (user?.instagramApi != undefined) {
      ...
      const mediaIds = (await InstagramMedia.findAll({where: { apiId: user.instagramApi.id }})).map(p => p.id);
      ...
      const existingApi = await InstagramApi.findOne({where: {nodeId: pageId}, paranoid: false});
      ...
      if (existingApi) {
        ...
        const existingApiMediaIds = (await InstagramMedia.findAll({where: { apiId: existingApi.id}, paranoid: false})).map(p => p.id);
        await InstagramMedia.restore({where: { id: existingApiMediaIds }});
        await InstagramComment.restore({where: { mediaId: existingApiMediaIds } });

      // First time setup
      } else {
        await InstagramApi.create({ facebookApiId: user!.facebookApi.id, userId: user!.id, nodeId: pageId });
      }
