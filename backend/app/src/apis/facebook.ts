...
export const getPages = async (userToken: string): Promise<{id: string, name: string, access_token: string}[]> => {
...
export const getPage = async (pageToken: string): Promise<{id: string, name: string}> => {
  const result = await axios.get('https://graph.facebook.com/v14.0/me', { params: { access_token: pageToken, fields: 'name' } });
  return result.data;
}